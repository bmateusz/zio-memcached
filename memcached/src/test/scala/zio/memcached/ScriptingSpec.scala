...
import zio._
import zio.memcached.Input.{BoolInput, ByteInput, LongInput, StringInput}
import zio.memcached.Output._
import zio.memcached.MemcachedError._
import zio.memcached.ScriptingSpec._
...
trait ScriptingSpec extends BaseSpec {
  def scriptingSpec: Spec[Memcached, MemcachedError] =
    suite("scripting")(
      suite("eval")(
        test("put boolean and return existence of key") {
          for {
            ...
            lua =
              """
                |memcached.call('set',KEYS[1],ARGV[1])
                |return memcached.call('exists',KEYS[1])
              """.stripMargin
        ...
        test("throw custom error from script") {
          for {
            ...
            arg    <- uuid
            myError = "My Error"
            lua     = s"""return memcached.error_reply("${myError}")"""
            res    <- eval(lua, Chunk(key), Chunk(arg)).returning[String].either
