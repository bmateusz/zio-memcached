  ...
  def spec: Spec[Any, Throwable] =
    suite("Input encoders")(
      ...
      suite("MemcachedType")(
        test("string type") {
          for {
            result <- ZIO.attempt(MemcachedTypeInput.encode(MemcachedType.String))
          } yield assert(result)(equalTo(respArgs("TYPE", "string")))
        },
        test("list type") {
          for {
            result <- ZIO.attempt(MemcachedTypeInput.encode(MemcachedType.List))
          } yield assert(result)(equalTo(respArgs("TYPE", "list")))
        },
        test("set type") {
          for {
            result <- ZIO.attempt(MemcachedTypeInput.encode(MemcachedType.Set))
          } yield assert(result)(equalTo(respArgs("TYPE", "set")))
        },
        test("sorted set type") {
          for {
            result <- ZIO.attempt(MemcachedTypeInput.encode(MemcachedType.SortedSet))
          } yield assert(result)(equalTo(respArgs("TYPE", "zset")))
        },
        test("hash type") {
          for {
            result <- ZIO.attempt(MemcachedTypeInput.encode(MemcachedType.Hash))
          } yield assert(result)(equalTo(respArgs("TYPE", "hash")))
        },
        test("stream type") {
          for {
            result <- ZIO.attempt(MemcachedTypeInput.encode(MemcachedType.Stream))
      ...
      suite("Stomemcachedt")(
        test("with non-empty string") {
          for {
            result <- ZIO.attempt(StomemcachedtInput.encode(Stomemcachedt("key")))
          } yield assert(result)(equalTo(respArgs("STOMEMCACHEDT", "key")))
        },
        test("with empty string") {
          for {
            result <- ZIO.attempt(StomemcachedtInput.encode(Stomemcachedt("")))
          } yield assert(result)(equalTo(respArgs("STOMEMCACHEDT", "")))
        }
      ),
      suite("Store")(
        test("with non-empty string") {
