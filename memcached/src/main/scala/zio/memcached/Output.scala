  ...
  private[memcached] final def unsafeDecode(respValue: RespValue)(implicit codec: Codec): A =
    respValue match {
      case RespValue.Error(msg) if msg.startsWith("ERR") =>
        throw MemcachedError.ProtocolError(msg.drop(3).trim)
      case RespValue.Error(msg) if msg.startsWith("WRONGTYPE") =>
        throw MemcachedError.WrongType(msg.drop(9).trim)
      case RespValue.Error(msg) if msg.startsWith("BUSYGROUP") =>
        throw MemcachedError.BusyGroup(msg.drop(9).trim)
      case RespValue.Error(msg) if msg.startsWith("NOGROUP") =>
        throw MemcachedError.NoGroup(msg.drop(7).trim)
      case RespValue.Error(msg) if msg.startsWith("NOSCRIPT") =>
        throw MemcachedError.NoScript(msg.drop(8).trim)
      case RespValue.Error(msg) if msg.startsWith("NOTBUSY") =>
        throw MemcachedError.NotBusy(msg.drop(7).trim)
      case RespValue.Error(msg) =>
        throw MemcachedError.ProtocolError(msg.trim)
      case success =>
        tryDecode(success)
    }
...
object Output {
  ...
  case object TypeOutput extends Output[MemcachedType] {
    protected def tryDecode(respValue: RespValue)(implicit codec: Codec): MemcachedType =
      respValue match {
        case RespValue.SimpleString("string") => MemcachedType.String
        case RespValue.SimpleString("list")   => MemcachedType.List
        case RespValue.SimpleString("set")    => MemcachedType.Set
        case RespValue.SimpleString("zset")   => MemcachedType.SortedSet
        case RespValue.SimpleString("hash")   => MemcachedType.Hash
        case RespValue.SimpleString("stream") => MemcachedType.Stream
        case other                            => throw ProtocolError(s"$other isn't memcached type.")
      }
  }
    ...
    override protected def tryDecode(respValue: RespValue)(implicit codec: Codec): Chunk[StreamGroupsInfo] =
      respValue match {
        ...
        case RespValue.Array(messages) =>
          messages.collect {
            // Note that you should not rely on the fields exact position. see https://memcached.io/commands/xinfo
            case RespValue.Array(elements) if elements.length % 2 == 0 =>
