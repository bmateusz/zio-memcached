...
import zio.memcached.RespValue.{BulkString, bulkString}
import zio.memcached.TestExecutor.{KeyInfo, KeyType}
import zio.stm._
...
private[memcached] final class TestExecutor private (
  ...
  override def execute(command: Chunk[RespValue.BulkString]): IO[MemcachedError, RespValue] =
    for {
  ...
  private[this] def runCommand(name: String, input: Chunk[RespValue.BulkString], now: Instant): USTM[RespValue] = {
    name match {
      ...
      case api.Geo.GeoRadius =>
        ...
        val stomemcachedt = stringInput.toList.sliding(2).collectFirst { case "STOMEMCACHEDT" :: key :: _ =>
          key
        }
          ...
          orWrongType(isSortedSet(key))(
            {
              for {
                ...
                _ <- stomemcachedt.fold(STM.unit)(key =>
                       putSortedSet(
                         key,
                ...
                if (store.nonEmpty || stomemcachedt.nonEmpty) RespValue.Integer(countedList.size.toLong)
      ...
      case api.Keys.Scan =>
        ...
        val memcachedTypeIndex = options.indexOf("TYPE")
        val memcachedType      = if (memcachedTypeIndex >= 0) Some(options(memcachedTypeIndex + 1)) else None
        ...
        for {
          ...
          filtered = memcachedType.map { rt =>
                       sliced.filter { case (_, info) => info.memcachedType.stringify == rt }
                     }.getOrElse(sliced)
      ...
      case api.Keys.TypeOf =>
        ...
        keys.get(key).map(info => info.map(_.memcachedType).fold("none")(_.stringify)).map(RespValue.SimpleString(_))
        ...
        orWrongType(isHash(key)) {
          ...
          if (count.isDefined && withValues) {
            ...
            for {
              ...
              key <- selectOne[zio.memcached.RespValue.BulkString](keys.toVector, randomPick)
...
private[memcached] object TestExecutor {
  ...
  object KeyType {
    ...
    def toMemcachedType(keyType: KeyType): MemcachedType = keyType match {
      case Strings    => MemcachedType.String
      case HyperLogs  => MemcachedType.String
      case Sets       => MemcachedType.Set
      case Lists      => MemcachedType.List
      case Hashes     => MemcachedType.Hash
      case SortedSets => MemcachedType.SortedSet
      case Stream     => MemcachedType.Stream
    }
  }

  final case class KeyInfo(`type`: KeyType, expireAt: Option[Instant]) {
    lazy val memcachedType: MemcachedType = KeyType.toMemcachedType(`type`)
  }
