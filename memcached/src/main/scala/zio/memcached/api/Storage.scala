...
trait Storage {
  ...
  final def touch(key: String,
                  expireTime: Duration): ZIO[Memcached, MemcachedError, Boolean] =
    MemcachedCommand(TouchCommand, TouchOutput).run((key, expireTime))

  final def getAndTouch[R: Schema](key: String,
                                   expireTime: Duration): ZIO[Memcached, MemcachedError, Option[R]] =
    MemcachedCommand(GatCommand, SingleGetOutput[R]()).run((key, expireTime))

  final def getAndTouchWithCas[R: Schema](key: String,
                                          expireTime: Duration): ZIO[Memcached, MemcachedError, Option[(CasUnique, R)]] =
    MemcachedCommand(GatsCommand, SingleGetWithCasOutput[R]()).run((key, expireTime))

  final def set[V: Schema](key: String,
                           value: V,
                           expireTime: Option[Duration] = None): ZIO[Memcached, MemcachedError, Boolean] =
    MemcachedCommand(new SetCommand[V](), SetOutput).run((key, value, expireTime))

  final def add[V: Schema](key: String,
                           value: V,
                           expireTime: Option[Duration] = None): ZIO[Memcached, MemcachedError, Boolean] =
    MemcachedCommand(new AddCommand[V](), SetOutput).run((key, value, expireTime))

  final def replace[V: Schema](key: String,
                               value: V,
                               expireTime: Option[Duration] = None): ZIO[Memcached, MemcachedError, Boolean] =
    MemcachedCommand(new ReplaceCommand[V](), SetOutput).run((key, value, expireTime))

  final def append[V: Schema](key: String,
                              value: V,
                              expireTime: Option[Duration] = None): ZIO[Memcached, MemcachedError, Boolean] =
    MemcachedCommand(new AppendCommand[V](), SetOutput).run((key, value, expireTime))

  final def prepend[V: Schema](key: String,
                               value: V,
                               expireTime: Option[Duration] = None): ZIO[Memcached, MemcachedError, Boolean] =
    MemcachedCommand(new PrependCommand[V](), SetOutput).run((key, value, expireTime))

  final def compareAndSet[V: Schema](key: String,
                                     value: V,
                                     casUnique: CasUnique,
                                     expireTime: Option[Duration] = None): ZIO[Memcached, MemcachedError, UpdateResult] =
    MemcachedCommand(new CompareAndSetCommand[V](), UpdateResultOutput).run((key, value, casUnique, expireTime))

  final def increase(key: String,
                     value: Long): ZIO[Memcached, MemcachedError, Long] =
    if (value >= 0)
