...
trait Lists {
   ...
   * available. This is the blocking variant of [[zio.memcached.api.Lists#rPopLPush]].
   *
   * @param source
  ...
  final def lIndex[K: Schema](key: K, index: Long): ResultBuilder1[Option] =
    new ResultBuilder1[Option] {
      def returning[V: Schema]: ZIO[Memcached, MemcachedError, Option[V]] =
        MemcachedCommand(LIndex, Tuple2(ArbitraryInput[K](), LongInput), OptionalOutput(ArbitraryOutput[V]()))
          .run((key, index))
    }
   ...
   */
  final def lLen[K: Schema](key: K): ZIO[Memcached, MemcachedError, Long] = {
    val command = MemcachedCommand(LLen, ArbitraryInput[K](), LongOutput)
    command.run(key)
  }
  ...
  final def lPop[K: Schema](key: K): ResultBuilder1[Option] =
    new ResultBuilder1[Option] {
      def returning[V: Schema]: ZIO[Memcached, MemcachedError, Option[V]] =
        MemcachedCommand(LPop, ArbitraryInput[K](), OptionalOutput(ArbitraryOutput[V]())).run(key)
    }
   ...
   */
  final def lPush[K: Schema, V: Schema](key: K, element: V, elements: V*): ZIO[Memcached, MemcachedError, Long] = {
    val command = MemcachedCommand(LPush, Tuple2(ArbitraryInput[K](), NonEmptyList(ArbitraryInput[V]())), LongOutput)
    command.run((key, (element, elements.toList)))
  }

  /**
   * Prepends an element to a list, only if the list exists. In contrary to [[zio.memcached.api.Lists#lPush]], no operation
   ...
   *   the length of the list after the push operation.
   */
  final def lPushX[K: Schema, V: Schema](key: K, element: V, elements: V*): ZIO[Memcached, MemcachedError, Long] = {
    val command = MemcachedCommand(LPushX, Tuple2(ArbitraryInput[K](), NonEmptyList(ArbitraryInput[V]())), LongOutput)
    command.run((key, (element, elements.toList)))
  }
  ...
  final def lRange[K: Schema](key: K, range: Range): ResultBuilder1[Chunk] =
    new ResultBuilder1[Chunk] {
      def returning[V: Schema]: ZIO[Memcached, MemcachedError, Chunk[V]] =
        MemcachedCommand(LRange, Tuple2(ArbitraryInput[K](), RangeInput), ChunkOutput(ArbitraryOutput[V]()))
          .run((key, range))
    }
   ...
   */
  final def lRem[K: Schema](key: K, count: Long, element: String): ZIO[Memcached, MemcachedError, Long] = {
    val command = MemcachedCommand(LRem, Tuple3(ArbitraryInput[K](), LongInput, StringInput), LongOutput)
   ...
   *   the Unit value.
   */
  final def lSet[K: Schema, V: Schema](key: K, index: Long, element: V): ZIO[Memcached, MemcachedError, Unit] = {
    val command = MemcachedCommand(LSet, Tuple3(ArbitraryInput[K](), LongInput, ArbitraryInput[V]()), UnitOutput)
   ...
   *   the Unit value.
   */
  final def lTrim[K: Schema](key: K, range: Range): ZIO[Memcached, MemcachedError, Unit] = {
    val command = MemcachedCommand(LTrim, Tuple2(ArbitraryInput[K](), RangeInput), UnitOutput)
  ...
  final def rPop[K: Schema](key: K): ResultBuilder1[Option] =
    new ResultBuilder1[Option] {
      def returning[V: Schema]: ZIO[Memcached, MemcachedError, Option[V]] =
        MemcachedCommand(RPop, ArbitraryInput[K](), OptionalOutput(ArbitraryOutput[V]())).run(key)
    }
  ...
  final def rPopLPush[S: Schema, D: Schema](source: S, destination: D): ResultBuilder1[Option] =
    new ResultBuilder1[Option] {
      def returning[V: Schema]: ZIO[Memcached, MemcachedError, Option[V]] =
        MemcachedCommand(RPopLPush, Tuple2(ArbitraryInput[S](), ArbitraryInput[D]()), OptionalOutput(ArbitraryOutput[V]()))
   ...
   *   the length of the list after the push operation.
   */
  final def rPush[K: Schema, V: Schema](key: K, element: V, elements: V*): ZIO[Memcached, MemcachedError, Long] = {
    val command = MemcachedCommand(RPush, Tuple2(ArbitraryInput[K](), NonEmptyList(ArbitraryInput[V]())), LongOutput)
    command.run((key, (element, elements.toList)))
  }

  /**
   * Appends on or multiple elements to the list stored at key, only if the list exists. In contrary to
   * [[zio.memcached.api.Lists#rPush]], no operation will be performed when key does not yet exist.
   *
   * @param key
   *   the key identifier
   * @param element
   ...
   *   the length of the list after the push operation.
   */
  final def rPushX[K: Schema, V: Schema](key: K, element: V, elements: V*): ZIO[Memcached, MemcachedError, Long] = {
    val command = MemcachedCommand(RPushX, Tuple2(ArbitraryInput[K](), NonEmptyList(ArbitraryInput[V]())), LongOutput)
    command.run((key, (element, elements.toList)))
  }
  ...
  final def blPop[K: Schema](key: K, keys: K*)(
    ...
    new ResultBuilder1[({ type lambda[x] = Option[(K, x)] })#lambda] {
      def returning[V: Schema]: ZIO[Memcached, MemcachedError, Option[(K, V)]] = {
        val command = MemcachedCommand(
          BlPop,
   ...
   * When used inside a MULTI/EXEC block, this command behaves exactly like [[zio.memcached.api.Lists#lMove]]. When source
   * is empty, Memcached will block the connection until another client pushes to it or until timeout is reached. A timeout
  ...
  /**
   * The command returns the index of matching elements inside a Memcached list. By default, when no options are given, it
  ...
  ): ZIO[Memcached, MemcachedError, Option[Long]] = {
    val command = MemcachedCommand(
      LPos,
      Tuple4(
        ArbitraryInput[K](),
  ...
  final def lPosCount[K: Schema, V: Schema](
    ...
    rank: Option[Rank] = None,
    maxLen: Option[ListMaxLen] = None
  ): ZIO[Memcached, MemcachedError, Chunk[Long]] = {
    val command = MemcachedCommand(
      LPos,
      Tuple5(
        ArbitraryInput[K](),
...
private[memcached] object Lists {
