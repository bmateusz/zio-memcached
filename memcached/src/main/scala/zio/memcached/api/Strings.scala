...
trait Strings {
   ...
   */
  final def append[K: Schema, V: Schema](key: K, value: V): ZIO[Memcached, MemcachedError, Long] = {
    val command = MemcachedCommand(Append, Tuple2(ArbitraryInput[K](), ArbitraryInput[V]()), LongOutput)
   ...
   */
  final def bitCount[K: Schema](key: K, range: Option[Range] = None): ZIO[Memcached, MemcachedError, Long] = {
    val command = MemcachedCommand(BitCount, Tuple2(ArbitraryInput[K](), OptionalInput(RangeInput)), LongOutput)
  ...
  ): ZIO[Memcached, MemcachedError, Chunk[Option[Long]]] = {
  ...
  final def bitOp[D: Schema, S: Schema](
    ...
    val command =
      MemcachedCommand(BitOp, Tuple3(BitOperationInput, ArbitraryInput[D](), NonEmptyList(ArbitraryInput[S]())), LongOutput)
  ...
  final def bitPos[K: Schema](
    ...
    val command =
      MemcachedCommand(BitPos, Tuple3(ArbitraryInput[K](), BoolInput, OptionalInput(BitPosRangeInput)), LongOutput)
   ...
   */
  final def decr[K: Schema](key: K): ZIO[Memcached, MemcachedError, Long] = {
    val command = MemcachedCommand(Decr, ArbitraryInput[K](), LongOutput)
    command.run(key)
  }
   ...
   *   Returns the value of key after the decrement.
   */
  final def decrBy[K: Schema](key: K, decrement: Long): ZIO[Memcached, MemcachedError, Long] = {
    val command = MemcachedCommand(DecrBy, Tuple2(ArbitraryInput[K](), LongInput), LongOutput)
  ...
  final def get[K: Schema](key: K): ResultBuilder1[Option] =
    new ResultBuilder1[Option] {
      def returning[R: Schema]: ZIO[Memcached, MemcachedError, Option[R]] =
        MemcachedCommand(Get, ArbitraryInput[K](), OptionalOutput(ArbitraryOutput[R]())).run(key)
    }
   ...
   */
  final def getBit[K: Schema](key: K, offset: Long): ZIO[Memcached, MemcachedError, Long] = {
    val command = MemcachedCommand(GetBit, Tuple2(ArbitraryInput[K](), LongInput), LongOutput)
  ...
  final def getRange[K: Schema](key: K, range: Range): ResultBuilder1[Option] =
    new ResultBuilder1[Option] {
      def returning[R: Schema]: ZIO[Memcached, MemcachedError, Option[R]] =
        MemcachedCommand(GetRange, Tuple2(ArbitraryInput[K](), RangeInput), OptionalOutput(ArbitraryOutput[R]()))
          .run((key, range))
    }
  ...
  final def getSet[K: Schema, V: Schema](key: K, value: V): ResultBuilder1[Option] =
    new ResultBuilder1[Option] {
      def returning[R: Schema]: ZIO[Memcached, MemcachedError, Option[R]] =
        MemcachedCommand(GetSet, Tuple2(ArbitraryInput[K](), ArbitraryInput[V]()), OptionalOutput(ArbitraryOutput[R]()))
          .run((key, value))
    }
  ...
  final def getDel[K: Schema](key: K): ResultBuilder1[Option] =
    new ResultBuilder1[Option] {
      def returning[R: Schema]: ZIO[Memcached, MemcachedError, Option[R]] =
        MemcachedCommand(GetDel, ArbitraryInput[K](), OptionalOutput(ArbitraryOutput[R]())).run(key)
    }
  ...
  final def getEx[K: Schema](key: K, expire: Expire, expireTime: Duration): ResultBuilder1[Option] =
    new ResultBuilder1[Option] {
      def returning[R: Schema]: ZIO[Memcached, MemcachedError, Option[R]] =
        MemcachedCommand(GetEx, GetExInput[K](), OptionalOutput(ArbitraryOutput[R]())).run((key, expire, expireTime))
    }
  ...
  final def getEx[K: Schema](key: K, expiredAt: ExpiredAt, timestamp: Instant): ResultBuilder1[Option] =
    new ResultBuilder1[Option] {
      def returning[R: Schema]: ZIO[Memcached, MemcachedError, Option[R]] =
        MemcachedCommand(GetEx, GetExAtInput[K](), OptionalOutput(ArbitraryOutput[R]())).run((key, expiredAt, timestamp))
    }
  ...
  final def getEx[K: Schema](key: K, persist: Boolean): ResultBuilder1[Option] =
    new ResultBuilder1[Option] {
      def returning[R: Schema]: ZIO[Memcached, MemcachedError, Option[R]] =
        MemcachedCommand(GetEx, GetExPersistInput[K](), OptionalOutput(ArbitraryOutput[R]())).run((key, persist))
    }
   ...
   */
  final def incr[K: Schema](key: K): ZIO[Memcached, MemcachedError, Long] = {
    val command = MemcachedCommand(Incr, ArbitraryInput[K](), LongOutput)
    command.run(key)
  }
   ...
   *   Returns the value of key after the increment.
   */
  final def incrBy[K: Schema](key: K, increment: Long): ZIO[Memcached, MemcachedError, Long] = {
    val command =
      MemcachedCommand(IncrBy, Tuple2(ArbitraryInput[K](), LongInput), LongOutput)
   ...
   *   Returns the value of key after the increment.
   */
  final def incrByFloat[K: Schema](key: K, increment: Double): ZIO[Memcached, MemcachedError, Double] = {
    val command = MemcachedCommand(IncrByFloat, Tuple2(ArbitraryInput[K](), DoubleInput), DoubleOutput)
    command.run((key, increment))
  }
  ...
  final def mGet[K: Schema](
    ...
    new ResultBuilder1[({ type lambda[x] = Chunk[Option[x]] })#lambda] {
      def returning[V: Schema]: ZIO[Memcached, MemcachedError, Chunk[Option[V]]] = {
        val command =
          MemcachedCommand(MGet, NonEmptyList(ArbitraryInput[K]()), ChunkOutput(OptionalOutput(ArbitraryOutput[V]())))
   ...
   */
  final def mSet[K: Schema, V: Schema](keyValue: (K, V), keyValues: (K, V)*): ZIO[Memcached, MemcachedError, Unit] = {
    val command = MemcachedCommand(MSet, NonEmptyList(Tuple2(ArbitraryInput[K](), ArbitraryInput[V]())), UnitOutput)
  ...
  final def mSetNx[K: Schema, V: Schema](
    ...
    val command = MemcachedCommand(MSetNx, NonEmptyList(Tuple2(ArbitraryInput[K](), ArbitraryInput[V]())), BoolOutput)
    command.run((keyValue, keyValues.toList))
  }
  ...
  final def pSetEx[K: Schema, V: Schema](
    ...
    val command =
      MemcachedCommand(PSetEx, Tuple3(ArbitraryInput[K](), DurationMillisecondsInput, ArbitraryInput[V]()), UnitOutput)
  ...
  final def set[K: Schema, V: Schema](
    ...
    val command = MemcachedCommand(Set, input, SetOutput)
   ...
   */
  final def setBit[K: Schema](key: K, offset: Long, value: Boolean): ZIO[Memcached, MemcachedError, Boolean] = {
    val command = MemcachedCommand(SetBit, Tuple3(ArbitraryInput[K](), LongInput, BoolInput), BoolOutput)
  ...
  final def setEx[K: Schema, V: Schema](
    ...
    val command =
      MemcachedCommand(SetEx, Tuple3(ArbitraryInput[K](), DurationSecondsInput, ArbitraryInput[V]()), UnitOutput)
   ...
   */
  final def setNx[K: Schema, V: Schema](key: K, value: V): ZIO[Memcached, MemcachedError, Boolean] = {
    val command = MemcachedCommand(SetNx, Tuple2(ArbitraryInput[K](), ArbitraryInput[V]()), BoolOutput)
    command.run((key, value))
  }
   ...
   */
  final def setRange[K: Schema, V: Schema](key: K, offset: Long, value: V): ZIO[Memcached, MemcachedError, Long] = {
    val command = MemcachedCommand(SetRange, Tuple3(ArbitraryInput[K](), LongInput, ArbitraryInput[V]()), LongOutput)
    command.run((key, offset, value))
  }
   ...
   */
  final def strLen[K: Schema](key: K): ZIO[Memcached, MemcachedError, Long] = {
    val command = MemcachedCommand(StrLen, ArbitraryInput[K](), LongOutput)
    command.run(key)
  }
  ...
  final def stralgoLcs[K: Schema](
  ...
  ): ZIO[Memcached, MemcachedError, LcsOutput] = {
    val memcachedCommand = MemcachedCommand(
      StrAlgoLcs,
      Tuple4(
    ...
    memcachedCommand.run((command.stringify, keyA, keyB, lcsQueryType))
  }
}

private[memcached] object Strings {
